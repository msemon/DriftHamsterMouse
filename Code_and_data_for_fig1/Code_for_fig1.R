...
# HAMSTER UPPER
model_hamupper<-readRDS("datamodel_HamUpper_for_fig1.rds")
cusp_hamupper<-read.table("datacusp_HamUpper_for_fig1.txt",sep="\t",h=T)
mini=min(cusp_hamupper$time)-0.00001
maxi=max(cusp_hamupper$time)
init=mini
pattern_hamupper=c(1,2,3,4,5,6,7)
plot_hamupper=plotmodel_bars(rep=c(5,5,5,5,5,5,5),pattern=pattern_hamupper,model=model_hamupper,title='Ham Upper',data=cusp_hamupper,end=maxi,init=mini)


# HAMSTER LOWER
model_hamlower<-readRDS("datamodel_HamLower_for_fig1.rds")
cusp_hamlower<-read.table("datacusp_HamLower_for_fig1.txt",sep="\t",h=T)
mini=min(cusp_hamlower$time)-0.00001
maxi=max(cusp_hamlower$time)
init=mini
pattern_hamlower=c(1,2,3,4,5,6,7)
plot_hamlower=plotmodel_bars(rep=c(5,5,5,5,5,5,5),pattern=pattern_hamlower,model=model_hamlower,title='Ham Lower',data=cusp_hamlower,end=maxi,init=mini)


# MOUSE LOWER
model_muslower<-readRDS("datamodel_MusLower_for_fig1.rds")
cusp_muslower<-read.table("datacusp_MusLower_for_fig1.txt",sep="\t",h=T)
pattern_muslower=c(1:7)
init=min(cusp_muslower$time)
mini=min(cusp_muslower$time)
maxi=max(cusp_muslower$time)
rep=c(5,5,5,5,5,5,5)
plot_muslower=plotmodel_bars(rep=c(5,5,5,5,5,5,5),pattern=pattern_muslower,model=model_muslower,title='Mus Lower',data=cusp_muslower,end=maxi,init=mini)

# MOUSE UPPER
model_musupper<-readRDS("datamodel_MusUpper_for_fig1.rds")
cusp_musupper<-read.table("datacusp_MusUpper_for_fig1.txt",sep="\t",h=T)
pattern_musupper=c(1:9)
init=min(cusp_musupper$time)
mini=min(cusp_musupper$time)
maxi=max(cusp_musupper$time)
plot_musupper=plotmodel_bars(rep=c(5,5,5,5,5,5,5,5,5),pattern=pattern_musupper,model=model_musupper,title='Mus Upper',data=cusp_musupper,end=maxi,init=mini)
...
ggsave("fig1_panelB.pdf",modelsannotate,width = 30, height = 30, units = "cm") 
