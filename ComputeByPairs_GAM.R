...
compute_timeeffect=function(facselec="machoire",selec="md",fac="espece")
{
  ...
  if(facselec=="espece")
  {
   # design <- as.formula(paste(" ~ machoire + (",
  #                             paste(spline_factors,collapse=" + "),
   #                            ") : ",fac," + 0 "))
  #  red.design <- as.formula(paste(" ~ machoire + (",
  #                                 paste(spline_factors,collapse=" + "),
   #                                ")  + 0 "))
  ...
  design <- as.formula(paste(" ~ espece + (",
                             paste(spline_factors,collapse=" + "),
                             ") : ",fac," + 0 "))
  
  
  
  red.design <- as.formula(paste(" ~ espece  + (",
                                 paste(spline_factors,collapse=" + "),
                                 ")  + 0 "))
  }
  ...
  dds <- DESeqDataSetFromMatrix(countData = counts(dds),
                                colData = cbind(df1[,c("espece","machoire","stade","rep","timerel")],SDM),
                                design = design)
  ...
  sc.sp1 <- results(dds, name=paste0("machoire","md"))[,2]
  sc.sp2 <- results(dds, name=paste0("machoire","mx"))[,2]
  ...
  sc.sp1 <- results(dds, name=paste0("espece","ham"))[,2]
  sc.sp2 <- results(dds, name=paste0("espece","mus"))[,2]
  names(sc.sp1)=row.names(dds)
  names(sc.sp2)=row.names(dds)
  }
  
  red.dds <- DESeqDataSetFromMatrix(countData=counts(dds),
                                    colData=cbind(df1[,c("machoire","stade","espece","machoire","timerel")],SDM),
                                    design=red.design)
  ...
  if(facselec!="espece")
  {
  sc.red.sp1 <- results(red.dds, name=paste0("espece","ham"))[,2]
  sc.red.sp2 <- results(red.dds, name=paste0("espece","mus"))[,2]
...
## to draw profiles 
profile <- function(gene="Pou3f3",facselec="espece",dds=dds,selec="md",res.diff,sc.sp1,sc.sp2,sc.fac1,sc.fac2,sc.red.sp1,sc.red.sp2,sc.red.t) {
  df1=colData(dds)
  times = seq(min(df1$timerel),max(df1$timerel),length.out=100)
  ...
  if(facselec!="espece")
  {
    dat[,levelsfac[1]] <- as.numeric(2 ** (sc.red.sp1[gene]+sc.fac1[gene,] %*% SDM))
    dat[,levelsfac[2]] <- as.numeric(2 ** (sc.red.sp2[gene]+sc.fac2[gene,] %*% SDM))
  ...
  dat0$mac=df1[,facselec]
  p=ggplot(dat0, aes(x=times, y=value, group=variable)) + 
    facet_grid(variable~mac , labeller = labeller(variable=sp_names,mac=to_names))+theme_bw() +
...
res_mus_vs_tooth=compute_timeeffect("espece","mus","machoire")
res_ham_vs_tooth=compute_timeeffect("espece","ham","machoire")

res_md_vs_sp=compute_timeeffect("machoire","md","espece")
res_mx_vs_sp=compute_timeeffect("machoire","mx","espece")


## to draw profiles tooth 
profileTooth <- function(gene="Pou3f3",resdds=res_md_vs_sp,facselec="machoire",selec="md") {
  ...
  dat0=data.frame(times=df1$timerel,value=k,variable=df1[,"machoire"],rep=df1[,"rep"],sp=df1[,"espece"])
  ...
  if(facselec!="espece")
  {
    ...
    dat0=data.frame(times=df1$timerel,value=k,variable=df1[,"espece"],rep=df1[,"rep"])
    dat0$mac=selec
  ...
  p=ggplot(dat0, aes(x=times, y=value, group=variable)) + 
    facet_grid(variable~mac, labeller=labeller(mac=to_names,variable=sp_names))+theme_bw() +
    geom_point(size=1.5,alpha = 0.2) + theme_bw(base_size = 12)+
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
    labs(x="Dev. time",y="Expr. level")+
    geom_line(data=models,aes(x=times, y=value,col=type,group = type))+
...
plot_mod_sp_vs_Tooth=function(gene,selec1="mus",selec2="ham",resdds1=res_mus_vs_tooth, resdds2=res_ham_vs_tooth,facsel="espece")
{
 # gene="Pou3f3"
...
plot_mod_sp_vs_Tooth("Bmp4",selec1="mus",selec2="ham",resdds1=res_mus_vs_tooth, resdds2=res_ham_vs_tooth,facsel="espece")
plot_mod_sp_vs_Tooth("Bmp4",selec1="md",selec2="mx",resdds1=res_md_vs_sp, resdds2=res_mx_vs_sp,facsel="machoire")

plot_mod_sp_vs_Tooth("Bmper",selec1="mus",selec2="ham",resdds1=res_mus_vs_tooth, resdds2=res_ham_vs_tooth,facsel="espece")
plot_mod_sp_vs_Tooth("Bmper",selec1="md",selec2="mx",resdds1=res_md_vs_sp, resdds2=res_mx_vs_sp,facsel="machoire")

plot_mod_sp_vs_Tooth("Fgf3",selec1="mus",selec2="ham",resdds1=res_mus_vs_tooth, resdds2=res_ham_vs_tooth,facsel="espece")
plot_mod_sp_vs_Tooth("Fgf3",selec1="md",selec2="mx",resdds1=res_md_vs_sp, resdds2=res_mx_vs_sp,facsel="machoire")


plot_mod_sp_vs_Tooth("Fgf10",selec1="mus",selec2="ham",resdds1=res_mus_vs_tooth, resdds2=res_ham_vs_tooth,facsel="espece")
plot_mod_sp_vs_Tooth("Fgf10",selec1="md",selec2="mx",resdds1=res_md_vs_sp, resdds2=res_mx_vs_sp,facsel="machoire")
...
plot_mod_sp_vs_Tooth("Wif1",selec1="mus",selec2="ham",resdds1=res_mus_vs_tooth, resdds2=res_ham_vs_tooth,facsel="espece")
plot_mod_sp_vs_Tooth("Wif1",selec1="md",selec2="mx",resdds1=res_md_vs_sp, resdds2=res_mx_vs_sp,facsel="machoire")


plot_mod_sp_vs_Tooth("Dkk1",selec1="mus",selec2="ham",resdds1=res_mus_vs_tooth, resdds2=res_ham_vs_tooth,facsel="espece")
plot_mod_sp_vs_Tooth("Dkk1",selec1="md",selec2="mx",resdds1=res_md_vs_sp, resdds2=res_mx_vs_sp,facsel="machoire")
